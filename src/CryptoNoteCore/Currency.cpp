...
namespace CryptoNote {
	...
	Difficulty Currency::nextDifficulty(std::vector<uint64_t> timestamps, std::vector<Difficulty> cumulativeDifficulties) const {
		//logger(INFO, BRIGHT_YELLOW) << "nextDifficulty[2]"; //ntc
		...
		// minimum limit
		if (nextDiff < DIFFICULTY_MIN) {
			nextDiff = DIFFICULTY_MIN;
		}

		int roz, last; //23456
		last = nextDiff % 10; //6 | 9
		roz = 8 - last; //2 | -1
		nextDiff = nextDiff + roz;
		//logger(INFO, BRIGHT_YELLOW) << "nextDiff " << nextDiff; //ntc
		return nextDiff;
	}
	...
	Difficulty Currency::nextDifficultyZawyV1(uint8_t version, uint32_t blockIndex, std::vector<uint64_t> timestamps, std::vector<Difficulty> cumulativeDifficulties) const {
		...
		uint64_t nextDiff = low / timeSpan;
	...
	Difficulty Currency::nextDifficultyZawyLWMA(uint8_t version, uint32_t blockIndex, std::vector<uint64_t> timestamps, std::vector<Difficulty> cumulativeDifficulties) const {
		...
		assert(n <= N);
		if (n <= 1) {
			logger(INFO, BRIGHT_YELLOW) << "nextDiff 1234"; //ntc
			return 1234;
		}
		...
		/* / minimum limit
		if (next_difficulty < DIFFICULTY_MIN) {
			next_difficulty = DIFFICULTY_MIN;
		}
		return next_difficulty;*/

		uint64_t nextDiff = next_difficulty;
	...
	Difficulty Currency::nextDifficultyDefault(uint8_t version, uint32_t blockIndex, std::vector<uint64_t> timestamps, std::vector<Difficulty> cumulativeDifficulties) const {
		...
		if (m_buggedZawyDifficultyBlockIndex && m_buggedZawyDifficultyBlockIndex <= blockIndex) {
			...
			// minimum limit
			if (nextDiffZ < DIFFICULTY_MIN) {
				nextDiffZ = DIFFICULTY_MIN;
			}
	...
	size_t Currency::getApproximateMaximumInputCount(size_t transactionSize, size_t outputCount, size_t mixinCount) const {
		...
		const size_t inputSize = INPUT_TAG_SIZE + AMOUNT_SIZE + KEY_IMAGE_SIZE + SIGNATURE_SIZE + GLOBAL_INDEXES_VECTOR_SIZE_SIZE + GLOBAL_INDEXES_INITIAL_VALUE_SIZE + mixinCount * (GLOBAL_INDEXES_DIFFERENCE_SIZE + SIGNATURE_SIZE);
